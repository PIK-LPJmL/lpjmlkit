...
**LPJmL Data** `r if(!knitr::is_latex_output()){":floppy_disk:"}` is a lpjmlkit
module that groups around the data class `LPJmLData` and aims to facilitate the
...
#### **1. `r if (!knitr::is_latex_output()) {":book:"}` Data reading function read_io**\
  \
...
#### **2. `r if (!knitr::is_latex_output()) {":file_folder:"}` Data class LPJmLData**\
  \
...
#### **3. `r if (!knitr::is_latex_output()) {":chart_with_upwards_trend:"}` Base stats of LPJmLData objects**\
  \
...
#### **4. `r if (!knitr::is_latex_output()) {":pencil2:"}` Modify LPJmLData objects**\
  \
  LPJmLData objects come with a bundle of methods to modify its state: 
  `add_grid()`, `transform()` and `subset()`.

* **`r if (!knitr::is_latex_output()) {":round_pushpin:"}` `add_grid()`** Add a **$grid** attribute (as LPJmLData object) to the
...
* **`r if (!knitr::is_latex_output()) {":repeat:"}` `transform()`** the `$data` dimensions.  
...
* **`r if (!knitr::is_latex_output()) {":scissors:"}` `subset()`** the `$data`.  
...
#### **5. `r if (!knitr::is_latex_output()) {":package:"}` Export LPJmLData objects**\
  \
...
* **`r if (!knitr::is_latex_output()) {":globe_with_meridians:"}` `as_raster()`** / **`as_terra()`** Export `$data` as a
