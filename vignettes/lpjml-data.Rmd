...
**LPJmL Data** `r if(!knitr::is_latex_output()){"&#128190;"}` is a lpjmlkit
...
#### **1. `r if (!knitr::is_latex_output()) {"&#128214;"}` Data reading function read_io**\
  \
...
#### **3. `r if (!knitr::is_latex_output()) {"&#128200;"}` Base stats of LPJmLData objects**\
  \
...
* **`r if (!knitr::is_latex_output()) {"&#128205;"}` `add_grid()`** Add a **$grid** attribute (as LPJmLData object) to the
