...
or common data formats can be exported (5).
...
* **"clm"** - *use if "meta" is not available or in combination*.  
  ...
  runoff <- read_i0("./output/mrunoff.clm",
...
* **`r if (!knitr::is_latex_output()) {"&#128205;"}` `add_grid()`** Add a
  **$grid** attribute (as LPJmLData object) to the object, providing the spatial
...
* **`r if (!knitr::is_latex_output()) {"&#128257;"}` `transform()`** the `$data` dimensions.  
  Transform spatial dimension from "cell" to "lon" (longitude) and "lat"
  (latitude) or temporal dimension "time" into separate "year", "month", and
  "day". Combinations and back transformations are also possible. Transformation
  into format "lon_lat" requires a `$grid` attribute (see `add_grid` above).
  Transformation does not change the contents of the data, only the structure.
  ```{r, echo = TRUE, eval = FALSE, highlight = TRUE}
...
* **`r if (!knitr::is_latex_output()) {"&#9986;"}` `subset()`** the `$data`.  
  Use `$data` dimensions as key and names or indices as value to subset `$data`.
...
* **`as_array()`** Export `$data` as an array. Beyond simply returning the
  `$data` element of an `LPJmLData` object, as_array provides additional
  functionality to subset and aggregate `$data`. Subsetting is conducted before
  aggregation.
  ```{r, echo = TRUE, eval = FALSE, highlight = TRUE}
...
#### **Miscellaneous** \
  ...
  * `read_meta()` to read meta information from meta and header files as 
    ...
    input or output file without reading the whole data.
...
2. Do not try to manually overwrite either the `$data` or any `$meta` data
  ...
  up the integrity of the object. Methods named like `$.__<method>__` or
  attributes named like `$._<attribute>_` are only for low-level package
  development and should not be used by users for their data handling.
...
4. The "meta" format is supported only by recent LPJmL versions. When comparing
