...
#' [`submit_lpjml()`] returns a tibble that can be
...
#'   of arguments gives the argument names. **IMPORTANT**: `fun` has to have an
#'   argument which acts as a placeholder to be filled with each *sim_name* of
#'   `x`/`select_sim`. If it should not be `x` please specify via
#'   `sim_name_arg`.
#'
...
#'   to a path name as `paste(output_path, "output", sim_name, sep = "/")` and
#'   passed this way to the `sim_name_arg`. Defaults to `FALSE`.
#'
...
#'   Defaults to `FALSE`.
#'
#' @param package_names Character vector providing package names of the
#'   functions to be used, or functions to be used inside if not called via
#'   package::function annotation. Defaults to `NULL`.
#'
#' @param memory Integer. Allows to overwrite default memory allocation based on
#'   `sclass` setting and allocate additional RAM if a function requires more
#'   than the default. Provide requested memory (in MB). Defaults to `NULL`.
#'
#' @param modules Character vector specifying modules to be loaded on the
#'   cluster. Please keep in mind that `module purge` is executed before the
#'   modules are loaded. In case of the default `NULL`, any modules that may
#'   already be loaded are not purged.
#'
#' @param no_submit Logical. Set to `TRUE` to test function without actually
#'   submitting a job. Defaults to `FALSE`.
#'
...
submit_postprocess <- function(x = NULL,
  ...
  # output_path is assumed to be location of output files if NULL is provided
...
submit_process <- function(x,
  ...
  if (!no_submit) {
    ...
    # Submit with processx to end all subprocesses at end and return parsable
