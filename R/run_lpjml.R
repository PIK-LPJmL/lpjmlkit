...
#' written. If `FALSE` (Default) these are printed instead. Within a SLURM Job
#' `write_stdout` is automatically set to `TRUE`.
#'
...
#' \link[tibble]{tibble} supplied as `param` to \link[lpjmlkit]{write_config}.
#'
...
# Conduct sequential runs (no parallelization), also switch of MPI on login
# node.
