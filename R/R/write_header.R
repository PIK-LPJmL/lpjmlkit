...
#' a list following the structure returned by read_header() or new_header().
...
#' @return None
#'
#' @seealso [new_header()] for creating headers from scratch and for a more detailed
#' description of the LPJmL header format, [read_header()] for reading headers from files.
#'
#' @examples
#' \dontrun{
#' header <- read_header(filename = "old_filename.clm")
#' write_header(filename = "new_filename.clm", header = header, overwrite = FALSE)
#' }
#'
#' @export
write_header <- function(filename, header, overwrite = FALSE) {
  ...
  if(anyNA(header$header[c("version", "order", "firstyear", "nyear", "firstcell", "ncell", "nbands")])) 
    stop(paste0("Header values must not be set to NA. Please check: ",
                toString(sQuote(names(which(is.na(header$header[c("version", "order", "firstyear", "nyear", "firstcell", "ncell", "nbands")])))))
               )
        )
  if(header$header["version"] > 1) {
    if(anyNA(header$header[c("cellsize_lon", "scalar")])) 
      stop(paste0("Header values must not be set to NA. Please check: ",
                  toString(sQuote(names(which(is.na(header$header[c("cellsize_lon", "scalar")])))))
                 )
          )
  }
  if(header$header["version"] > 2) {
    if(anyNA(header$header[c("cellsize_lat", "datatype")])) 
      stop(paste0("Header values must not be set to NA. Please check: ",
                  toString(sQuote(names(which(is.na(header$header[c("cellsize_lat", "datatype")])))))
                 )
          )
  }
