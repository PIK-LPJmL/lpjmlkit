...
LPJmLGridData$set(
  ...
  function(to = NULL) {
    ...
    if (private$.meta$._space_format_ == "cell" &&
      ...
      colnames(grid_extent) <- dimnames(self$data)$band
      ...
      ilon <- round(
        (asub(self$data, band = "lon") - grid_extent["min", "lon"]) /
          private$.meta$cellsize_lon
      ) + 1
      # ilat in descending order
      ilat <- round(
        (grid_extent["max", "lat"] - asub(self$data, band = "lat")) /
      ...
      grid_array[cbind(ilon, ilat, 1)] <- as.integer(dimnames(self$data)$cell)
      ...
      grid_dimnames <- lapply(dimnames(self$data)[c("lon", "lat")], as.numeric)
